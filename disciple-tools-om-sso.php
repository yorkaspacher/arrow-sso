<?php
/**
 * Plugin Name: Disciple Tools - OM SSO
 * Plugin URI: https://github.com/DiscipleTools/disciple-tools-om-sso
 * Description: Disciple Tools - OM SSO is intended to help offer Single-Sign-On (SSO) authentication through Microsoft Office 365.
 * Text Domain: disciple-tools-om-sso
 * Domain Path: /languages
 * Version:  1.0
 * Author URI: https://github.com/DiscipleTools
 * GitHub Plugin URI: https://github.com/DiscipleTools/disciple-tools-om-sso
...
/**
 * Gets the instance of the `Disciple_Tools_OM_SSO` class.
 *
 * @since  0.1
 * @access public
 * @return object|bool
 */
function disciple_tools_om_sso() {
    $disciple_tools_om_sso_required_dt_theme_version = '1.0';
    ...
    if ( $is_theme_dt && version_compare( $version, $disciple_tools_om_sso_required_dt_theme_version, "<" ) ) {
        add_action( 'admin_notices', 'disciple_tools_om_sso_hook_admin_notice' );
    ...
    return Disciple_Tools_OM_SSO::instance();

}
add_action( 'after_setup_theme', 'disciple_tools_om_sso', 20 );
...
class Disciple_Tools_OM_SSO {
    ...
    private function __construct() {
         ...
         */
        if ( $is_rest && strpos( dt_get_url_path(), 'disciple_tools_om_sso_template' ) !== false ) {
         ...
         *
        if ( strpos( dt_get_url_path(), 'metrics' ) !== false || ( $is_rest && strpos( dt_get_url_path(), 'disciple-tools-om-sso-metrics' ) !== false ) ){
        ...
        /**
         * SSO related classes and files
         */
        require_once( 'sso/sso-login.php' );
    ...
    public static function deactivation() {
        ...
        delete_option( 'dismissed-disciple-tools-om-sso' );
    }
    ...
    public function __call( $method = '', $args = array() ) {
        _doing_it_wrong( "disciple_tools_om_sso::" . esc_html( $method ), 'Method does not exist.', '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
register_activation_hook( __FILE__, [ 'Disciple_Tools_OM_SSO', 'activation' ] );
register_deactivation_hook( __FILE__, [ 'Disciple_Tools_OM_SSO', 'deactivation' ] );


if ( ! function_exists( 'disciple_tools_om_sso_hook_admin_notice' ) ) {
    function disciple_tools_om_sso_hook_admin_notice() {
        global $disciple_tools_om_sso_required_dt_theme_version;
        $wp_theme = wp_get_theme();
        $current_version = $wp_theme->version;
        $message = "'Disciple Tools - OM SSO' plugin requires 'Disciple Tools' theme to work. Please activate 'Disciple Tools' theme or make sure it is latest version.";
        if ( $wp_theme->get_template() === "disciple-tools-theme" ){
            $message .= ' ' . sprintf( esc_html( 'Current Disciple Tools version: %1$s, required version: %2$s' ), esc_html( $current_version ), esc_html( $disciple_tools_om_sso_required_dt_theme_version ) );
        }
        // Check if it's been dismissed...
        if ( ! get_option( 'dismissed-disciple-tools-om-sso', false ) ) { ?>
            <div class="notice notice-error notice-disciple-tools-om-sso is-dismissible" data-notice="disciple-tools-om-sso">
                <p><?php echo esc_html( $message );?></p>
            </div>
            <script>
                jQuery(function($) {
                    $( document ).on( 'click', '.notice-disciple-tools-om-sso .notice-dismiss', function () {
                        $.ajax( ajaxurl, {
                            type: 'POST',
                            data: {
                                action: 'dismissed_notice_handler',
                                type: 'disciple-tools-om-sso',
 ...
 * @link https://github.com/DiscipleTools/disciple-tools-om-sso/wiki/Configuring-Remote-Updating-System
 *
...
//                'https://raw.githubusercontent.com/DiscipleTools/disciple-tools-om-sso/master/version-control.json',
//                __FILE__,
//                'disciple-tools-om-sso'
//            );
//
//        }
//    }
